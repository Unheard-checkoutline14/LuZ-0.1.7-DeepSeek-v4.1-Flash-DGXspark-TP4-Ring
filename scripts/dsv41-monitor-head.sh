# [RETIRED-20260920] 名义自愈 monitor，从未接线（ROOT=/home/spark 不存在、worker 主机为占位符、无 systemd/cron 引用）。
# 生产自愈现役路径 = systemd 用户单元 dsv41-serve.service（head=launcher-waiter / worker=waiter）+ oom-gate-monitor。
# 保留本文件仅作历史参考；勿手工运行。
