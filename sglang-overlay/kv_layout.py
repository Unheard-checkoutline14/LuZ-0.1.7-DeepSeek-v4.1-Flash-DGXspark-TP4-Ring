...
def is_valid_fork_kv_layout_pair(kv: KVLayout, extra_kv: KVLayout) -> bool:
    """FORK (SM120/SM121 FlashInfer readers): upstream pairs plus a V4 (584 B)
    main cache with fp4 compressed caches. The FlashInfer sparse-MLA kernel only
    reads the 584-byte V4 layout, so the fp4 compressed pools are consumed
    through the out-of-kernel repack bridge (dequant_k_cache -> V4 scratch),
    which makes the (V4, V41_FP4) pair valid on our fleet even though the
    upstream FlashMLA SM100 kernel would reject it."""
    return is_valid_kv_layout_pair(kv, extra_kv) or (
        kv is KVLayout.V4 and extra_kv is KVLayout.V41_FP4
    )
