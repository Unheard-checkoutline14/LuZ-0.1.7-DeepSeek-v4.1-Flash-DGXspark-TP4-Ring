...
def _deepseek_v4_kv_cache_dtype(view: Any) -> dict:
    ...
    # FORK (M0 FP4 main-KV port of sglang PR #39123): ``fp4_mx_block16`` is an
    # alias for the V41_FP4 compressed-cache layout (e2m1 payload + per-16
    # e4m3 scales). The pool buffers stay uint8 / fp8-viewed, so the dtype is
    # normalized back to fp8_e4m3 and the layout is carried by the env the
    # memory-pool reads (SGLANG_DSV4_KV_LAYOUT=fork-v4-fp4: 584 B V4 main/SWA
    # caches + 288 B fp4 ratio-1/2 compressed caches on SM120/SM121).
    fp4_main_kv = False
    if kv_cache_dtype == "fp4_mx_block16":
        if view.device == "npu":
            raise ValueError(
                "fp4_mx_block16 (V41_FP4 compressed caches) is not supported on NPU"
            )
        fp4_main_kv = True
        kv_cache_dtype = "fp8_e4m3"
        logger.warning(
            f"kv_cache_dtype=fp4_mx_block16 for {model_arch}: storing the ratio-1/2 "
            "compressed KV as V41_FP4 (e2m1 + per-16 e4m3, 288 B/token); the main/SWA "
            "caches stay on the 584 B V4 layout and fp4 pools are read through the "
            "repack bridge."
        )
    ...
    if fp4_main_kv:
        ...
        os.environ.setdefault("SGLANG_DSV4_KV_LAYOUT", "fork-v4-fp4")
    if kv_cache_dtype != view.kv_cache_dtype:
        return {"kv_cache_dtype": kv_cache_dtype}
    return {}
