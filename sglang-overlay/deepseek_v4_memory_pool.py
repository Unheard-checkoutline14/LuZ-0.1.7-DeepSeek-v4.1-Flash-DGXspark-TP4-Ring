...
def resolve_compressed_kv_layout(
    ...
    if kv_layout is KVLayout.V4:
        if option == "fp4":
            # FORK (SM120/SM121): the FlashInfer reader keeps the 584-byte V4
            # main/SWA layout and the fp4 compressed pools are read through the
            # repack bridge, so this pair is valid here even though upstream's
            # FlashMLA SM100 kernel rejects it. Ratio-4/128 latents are not
            # fp4-rounded by the model and stay V4.
            return KVLayout.V41_FP4 if compress_ratio in (1, 2) else KVLayout.V4
        assert option is None, "the V4 main cache only pairs with V4 caches"
        return KVLayout.V4
...
def select_dsv4_kv_layout() -> Tuple[KVLayout, Optional[str]]:
    ...
    FORK extension (SM120/SM121): ``fork-v4-fp4`` keeps the 584-byte V4 main /
    SWA caches (the only layout our FlashInfer sparse-MLA reader accepts) and
    stores the ratio-1 / ratio-2 compressed latents as V41_FP4 (288 B/token,
    e2m1 + per-16 e4m3). The fp4 pools are consumed through the out-of-kernel
    repack bridge, not by the FlashInfer kernel, so no SM100 gate applies.
    ``--kv-cache-dtype fp4_mx_block16`` is an alias that selects this mode.
    """
    mode = envs.SGLANG_DSV4_KV_LAYOUT.get().lower()
    option = envs.SGLANG_DSV4_COMPRESSED_KV_LAYOUT.get().lower()
    if mode == "fork-v4-fp4":
        return KVLayout.V4, "fp4"
    if mode == "v4":
...
class DeepSeekV4TokenToKVPool(BaseSWAKVPool):
    ...
    def compressed_kv_layout(self, compress_ratio: int) -> KVLayout:
        ...
        assert is_valid_fork_kv_layout_pair(self.kv_layout, layout)
        return layout
