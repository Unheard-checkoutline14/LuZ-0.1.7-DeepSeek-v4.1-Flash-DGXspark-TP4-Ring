...
class DSV4PoolConfigurator(MemoryPoolConfigurator):
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        # FORK (M0 FP4 main-KV port of sglang PR #39123): under
        # SGLANG_DSV4_KV_LAYOUT=fork-v4-fp4 the ratio-1/2 latents are stored as
        # V41_FP4 (e2m1 + per-16 e4m3, 288 B/token) instead of the 584 B V4
        # layout; the SWA / c4 / c128 pools keep kv_bytes. Mirror
        # deepseek_v4_memory_pool.resolve_compressed_kv_layout so the capacity
        # estimate matches the pools that are actually allocated.
        low_ratio_kv_bytes = self.kv_bytes
        _fork_layout = envs.SGLANG_DSV4_KV_LAYOUT.get().lower()
        if _fork_layout in ("fork-v4-fp4", "v41"):
            _fork_option = envs.SGLANG_DSV4_COMPRESSED_KV_LAYOUT.get().lower()
            _fp4_low_ratio = (
                _fork_layout == "fork-v4-fp4" and _fork_option in ("auto", "fp4")
            ) or (_fork_layout == "v41" and _fork_option == "fp4")
            if _fp4_low_ratio:
                low_ratio_kv_bytes = 256 + 32  # V41_FP4 bytes_per_token
                # Fail fast: the fp4 compressed pools are readable only through
                # the SM120/121 repack bridge. Every other reader (the q8kv8
                # prefill gather, the non-SM120 sparse-MLA fallback) reads the
                # 584-byte V4 form and would silently consume fp4 bytes.
                assert get_platform().is_sm120, (
                    "SGLANG_DSV4_KV_LAYOUT=fork-v4-fp4 (V41_FP4 compressed KV) "
                    "requires the SM120/121 sparse-MLA reader; the repack bridge "
                    "is SM120-only"
                )
        if low_ratio_kv_bytes != self.kv_bytes:
            logger.info(
                f"DSV4 low-ratio compressed KV stored as V41_FP4: "
                f"{low_ratio_kv_bytes} B/token (was {self.kv_bytes})"
            )
        self.low_ratio_bytes_per_full_token = sum(
            (low_ratio_kv_bytes + low_ratio_index_bytes) / cfg.compress_ratios[l]
