...
class _StagedTokenInputs(NamedTuple):
    """QW3 (PR #39154 port): the token inputs the full load_batch path last
    copied into the graph buffers; a weak batch ref so the runner never
    extends a batch's life. Active only under
    SGLANG_QW3_DSPARK_LAUNCH_PARAMS."""
...
class DecodeCudaGraphRunner(BaseCudaGraphRunner):
    ...
    def __init__(
        ...
        *,
        ...
        # QW3 (PR #39154 port): identity record of the token inputs the full
        # load_batch path staged into the graph buffers, so the pre-planned
        # fast path can skip its redundant re-copies. Stays None (and the skip
        # never fires) unless SGLANG_QW3_DSPARK_LAUNCH_PARAMS is set.
    ...
    def _ragged_capture_slots(self, num_tokens: int) -> int:
        # Upstream PR #39257: token tiers are built as capture_bs *
        # captured_req_width. Preserve that request geometry for capture,
        # replay padding and admission (fixes sgl-project/sglang#39173,
        # the compact ragged-verify x engram target-verify assertion).
    ...
    def load_batch(
        ...
        if envs.SGLANG_QW3_DSPARK_LAUNCH_PARAMS.get():
            # QW3: the full path just staged this batch's tokens; a later
            # fast-path load of the same batch can skip its own copies.
    ...
    def _token_inputs_staged(self, forward_batch: ForwardBatch) -> bool:
        """QW3 (PR #39154 port): whether the full load_batch path already
        copied this batch's input_ids/positions into the graph buffers, so
        the pre-planned fast path can skip its own copies.
