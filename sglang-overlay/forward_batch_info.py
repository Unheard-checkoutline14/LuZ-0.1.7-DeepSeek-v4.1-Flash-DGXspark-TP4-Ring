...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    # DSV41 opt1 (skip non-final decoder chunks): per-request True when this
    # extend chunk finishes the request (scheduler-computed; see
    # ScheduleBatch.prepare_for_extend). The DSV4 backend uses it to restrict
    # the late-layer (post-last-kv_source) tail forward to final-chunk rows.
    # None -> cover every request (base behavior).
    ...
    def init_new(
        ...
        *,
        ...
        if ret.forward_mode.is_decode() or ret.forward_mode.is_target_verify():
            ...
            # DSV41 opt1: carry the scheduler-side final-chunk marking through
            # to the DSV4 attention backend (None keeps base behavior).
            ret.extend_last_chunk_mask = getattr(
                batch, "extend_last_chunk_mask", None
            )
