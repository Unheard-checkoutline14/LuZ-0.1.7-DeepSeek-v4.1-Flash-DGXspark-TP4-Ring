...
def quantize_k_cache_v41_fp4(
    ...
    ``scale = e4m3(clamp(amax * (1/6), 2**-9, 448))`` -- the reciprocal form, so
    this matches the CUDA writer bit for bit (`store_row_fp4` ->
    `fp4::compressed_kv_scale`; a true division rounds to a different e4m3 byte
    for ~1 in 2^23 amax values). A non-finite amax does not propagate: the
    writer clamps it (NaN to the 2**-9 floor, +-inf to 448), as here."""
    num_pages, page_size, d = k.shape
    assert d == 512
    x = k.float().view(num_pages, page_size, 32, 16)
    # The writer reduces the tile with `fmaxf`, which drops NaN operands: a NaN
    # element contributes nothing to the tile max, and only an all-NaN tile
    # reaches `compressed_kv_scale` as NaN -- where `fmaxf(NaN, 2**-9)` picks the
    # floor. Dropping NaNs to 0 here (rather than letting `amax` propagate them,
    # as plain `torch.amax` would) reproduces both cases, and lets +-inf reach the
    # clamp so it lands on 448 as the writer's `fminf` does.
    amax = torch.nan_to_num(x.abs(), nan=0.0, posinf=float("inf")).amax(dim=-1)
    scale = torch.clamp(amax * (1.0 / FP4_MAX), 2.0**-9, 448.0).to(torch.float8_e4m3fn)
    # `+ 0.0` clears a `-0.0` payload, as the writer's `__fdiv_rn(...) + 0.0f`
    # does: the sign would otherwise survive into the code as ``0x8``.
    codes = quantize_to_e2m1_codes(x / scale.float().unsqueeze(-1) + 0.0)
