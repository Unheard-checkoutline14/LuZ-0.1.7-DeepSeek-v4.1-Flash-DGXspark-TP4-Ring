...
class Envs:
    ...
    # QW3 (r9-ops 2026-09-15, PR #39154 port): eliminate the DSpark decode
    # per-step pinned H2D launches -- (a) the draft ForwardBatch's
    # num_token_non_padded scalar becomes a persistent device tensor refilled
    # in place (fill_ kernel) instead of a per-step pinned alloc + H2D copy,
    # and (b) the per-step decode plan (cur/nxt page-aligned KV bounds) is
    # staged through a per-device 2-deep pinned ring whose H2D is skipped on
    # rounds no consumed row changed (graph runner also skips the redundant
    # input_ids/positions re-copy on the pre-planned fast path). Default off:
    # byte-identical behavior to the pre-QW3 fork.
    SGLANG_QW3_DSPARK_LAUNCH_PARAMS = EnvBool(False)
    ...
    # fp4 compressed caches), "auto" (v41 on SM100 when FlashMLA supports it) or
    # FORK "fork-v4-fp4" (SM120/SM121: 584 B V4 main/SWA caches + 288 B fp4
    # ratio-1/2 compressed caches, read via the repack bridge).
    ...
    # FORK M1: memory budget (MiB) of the persistent V4 repack-bridge scratch
    # that turns V41_FP4 top-k slots into the 584 B rows the SM120/121
    # FlashInfer kernel reads. Batches whose tokens x topk exceed the budget
    # are query-sliced (exact: the softmax is per query token). Decode/verify
    # need ~43 MiB at bs 12 x draft 6 x topk 1024.
    SGLANG_DSV4_FP4_BRIDGE_SCRATCH_MB = EnvInt(512)

    # Bridge telemetry (measurement only; 0 = off). Every N bridge calls, log
    # rows / real rows (prefill only: summing topk_lengths needs a host sync,
    # illegal inside graph capture) / slice counts / bridge+attention us.
    # Exists to price the three candidate fixes from data instead of arithmetic.
    DSV41_FP4_BRIDGE_STATS = EnvInt(0)

    # Dense indexer logits row-chunk budget, bytes of one bf16 [rows, lc] score
    # slice (see the constant of the same name in deepseek_v4_backend.py, which
    # is what reads this). Prefill transients run ~3x the budget, so this is the
    # knob that sets how deep the long-prompt memory valley goes:
    #   1<<31 (2 GiB, historical) -> ~6 GiB transients on a 4x245K load
    #   1<<29 (512 MiB)           -> ~1.5 GiB, at the cost of 4x the loop
    #                                iterations over the same total work
    # Community data point (sglang #36941, 2xGB10): forcing 512-row logits
    # chunking turned a >300s timeout into a 135s PASS.
    # Unset = historical value (byte-for-byte old behaviour).
    DSV41_DENSE_INDEXER_LOGITS_BUDGET_BYTES = EnvInt(1 << 31)
