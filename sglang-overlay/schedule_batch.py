...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    # DSV41 opt1 (skip non-final decoder chunks): per-request flag, True when
    # this extend chunk finishes the request's prefill (or the request must be
    # treated as final, e.g. it asks for logprobs inside this chunk). The DSV4
    # backend restricts the late-layer tail forward to the True rows; False
    # rows are dead output (later chunks overwrite their window slots and
    # every late-layer reader is floored at its own chunk's tail start).
    # None when unknown (decode / gpu_only paths) -> base behavior.
    extend_last_chunk_mask: Optional[List[bool]] = None
    ...
    def prepare_for_extend(self):
        ...
        # DSV41 opt1: per-request "this chunk finishes the request's prefill".
        # extend_range.end is where this chunk stops; full_untruncated_fill_ids
        # is the whole planned fill (the PrefillAdder chunks by extend_range).
        # Requests asking for logprobs of tokens inside this chunk are
        # force-marked True so the model-side bounded-replay rejection
        # (_check_late_layer_tail_readers) still fires exactly as before.
        self.extend_last_chunk_mask = [
            (
                r.extend_range.end >= len(r.full_untruncated_fill_ids)
                or (
                    self.return_logprob
                    and extend_logprob_start_lens[i] < extend_lens[i]
                )
            )
            for i, r in enumerate(reqs)
        ]
    ...
    def mix_with_running(self, running_batch: ScheduleBatch):
        ...
        # DSV41 opt1: decode reqs riding a chunked-prefill batch contribute
        # 1-token extends that always finish their current round -> final.
        self.extend_last_chunk_mask = (self.extend_last_chunk_mask or []) + [
            True
        ] * running_bs
