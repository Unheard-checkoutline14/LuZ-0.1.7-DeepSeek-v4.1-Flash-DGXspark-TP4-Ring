...
# Dense (full-prefix) indexer logits row-chunk budget, in bytes of a bf16
# [rows, lc] score slice. Read from env so the slicing granularity can be
# A/B'd in a single-variable window without editing this file, and so a
# regression is one config line away from rollback:
#   smaller -> the prefill transient valley shrinks (live transients run ~3x
#              this budget), at the cost of more loop iterations over the
#              *same* total work
#   larger  -> fewer iterations, bigger valley (1<<31 was the historical value)
# Default is the historical value, so an unset env is byte-for-byte the old
# behaviour. Floored at 1 MiB because rows_per_chunk is `max(1, ...)`: a
# near-zero budget would silently degrade to one row per iteration (4096
# iterations per 4096-token chunk) instead of failing loudly.
_DENSE_INDEXER_LOGITS_BUDGET_BYTES = max(
    1 << 20, int(envs.DSV41_DENSE_INDEXER_LOGITS_BUDGET_BYTES.get())
)
...
class LateLayerTail(msgspec.Struct, frozen=True):
    ...
    # DSV41 opt1 mixed batch: the same per-request tail lens re-keyed onto the
    # FULL batch (0 at requests dropped from this tail). The dense prefill
    # indexer and the candidate-mask cut account over the full batch
    # (forward_batch.seq_lens_cpu / req_pool_indices), so a subset tail must
    # present bs-aligned lens: len(q_lens) == len(starts) == bs and
    # sum(q_lens) == subset row count (dropped requests repeat 0 times).
    # None unless this tail covers a strict subset of the batch (base behavior).
    full_batch_tail_lens: Optional[torch.Tensor] = None
    full_batch_tail_lens_cpu: Optional[List[int]] = None
...
class DeepseekV4AttnBackend(
    ...
    def __init__(
        ...
        # DSV41 opt1 (skip non-final decoder chunks): when enabled, extend
        # chunks that do not finish their request skip the late-layer tail
        # forward entirely (pure middle-chunk batches) or restrict it to the
        # final-chunk requests' rows (mixed batches). Opt-in via env so the
        # default is bit-identical to the base engine; DSV41_SKIP_NONFINAL_
        # DECODER=0 is the one-switch rollback.
        self.skip_nonfinal_decoder_tail: bool = get_bool_env_var(
            "DSV41_SKIP_NONFINAL_DECODER", "false"
        )
        # True iff the current forward's tail metadata was skipped because no
        # request in the batch finishes its prefill in this chunk; the model
        # reads this to skip the late layers instead of running them over the
        # full sequence (which would be slower than the tail itself).
        self.late_layer_tail_skip: bool = False
        ...
        self.prefill_reuse_preset = resolve_prefill_reuse_preset(
            model_runner.server_args.deepseek_v4_prefill_reuse
        )
    ...
    def _build_late_layer_tail_metadata(
        self,
        forward_batch: ForwardBatch,
        final_req_mask: Optional[List[bool]] = None,
    ) -> DSV4Metadata:
        ...
        final_req_mask (DSV41 opt1) restricts the tail to the requests marked
        True -- those whose current chunk finishes them. Non-final chunks'
        tail rows are dead output: later chunks overwrite their window slots
        and every late-layer reader is floored at its own chunk's tail start.
        None (or all-True) keeps every request, i.e. base behavior."""
        extend_lens_cpu = forward_batch.extend_seq_lens_cpu
        seq_lens_cpu = forward_batch.seq_lens_cpu
        assert extend_lens_cpu is not None and seq_lens_cpu is not None
        if final_req_mask is not None and not all(final_req_mask):
            keep = [i for i, m in enumerate(final_req_mask) if m]
            sub_extend_lens_cpu = [int(extend_lens_cpu[i]) for i in keep]
            sub_seq_lens_cpu = [int(seq_lens_cpu[i]) for i in keep]
            full_tail_lens_cpu: Optional[List[int]] = None  # filled below
        else:
            keep = None
            sub_extend_lens_cpu = [int(n) for n in extend_lens_cpu]
            sub_seq_lens_cpu = seq_lens_cpu.tolist()
            full_tail_lens_cpu = None
        device = forward_batch.out_cache_loc.device
        token_indices, tail_lens_cpu, swa_replay_start = late_layer_tail_layout(
            extend_lens_cpu=sub_extend_lens_cpu,
            seq_lens_cpu=sub_seq_lens_cpu,
            tail_len=SWA_WINDOW,
            device=device,
        )
        if keep is not None:
            # late_layer_tail_layout indexes the subset's concatenated rows
            # (0-based over the kept requests); shift each kept request's rows
            # by (full_batch_start - subset_start) so LateLayerTail consumers
            # (tail.rows) gather from full-batch tensors.
            full_starts = [0]
            for n in extend_lens_cpu[:-1]:
                full_starts.append(full_starts[-1] + n)
            sub_starts = [0]
            for n in sub_extend_lens_cpu[:-1]:
                sub_starts.append(sub_starts[-1] + n)
            shifts = torch.tensor(
                [full_starts[i] - s for i, s in zip(keep, sub_starts)], device=device
            ).to(token_indices.dtype)
            req_of_row = torch.repeat_interleave(
                torch.arange(len(keep), device=device),
                torch.tensor(tail_lens_cpu, device=device),
                output_size=token_indices.shape[0],
            )
            token_indices = token_indices + shifts[req_of_row]
            # Re-key the subset's per-request tail lens onto the full batch
            # (0 at dropped requests). _low_ratio_index_topk_dense lays starts,
            # lc_per_req and req_pool_indices over the FULL forward_batch, so
            # its ks repeat_interleave needs len(repeats) == len(starts) == bs
            # with sum(repeats) == subset row count; dropped requests repeat 0
            # times and hit the t_len == 0 branch in publish/consume.
            full_tail_lens_cpu = [0] * len(extend_lens_cpu)
            for i, n in zip(keep, tail_lens_cpu):
                full_tail_lens_cpu[i] = n
        contiguous_start = (
            extend_lens_cpu[0] - tail_lens_cpu[0]
            if keep is None and len(extend_lens_cpu) == 1
            else None
        )
        if keep is None:
            sub_req_pool_indices = forward_batch.req_pool_indices
            sub_seq_lens = forward_batch.seq_lens.to(torch.int32)
        else:
            keep_t = torch.tensor(keep, device=device, dtype=torch.long)
            sub_req_pool_indices = forward_batch.req_pool_indices[keep_t]
            sub_seq_lens = forward_batch.seq_lens[keep_t].to(torch.int32)
        ...
        metadata = self.init_forward_metadata_prefill(
            max_seq_len=max(sub_seq_lens_cpu),
            req_pool_indices=sub_req_pool_indices,
        ...
        metadata.low_ratio_req_indices = torch.repeat_interleave(
            sub_req_pool_indices.to(torch.int64), tail_lens.to(torch.int64)
        )
        ...
        if cp_tail is None:
            ...
            metadata.late_layer_tail = LateLayerTail(
                ...
                contiguous_start=contiguous_start,
                full_batch_tail_lens=(
                    torch.tensor(
                        full_tail_lens_cpu, dtype=torch.int32, device=device
                    )
                    if full_tail_lens_cpu is not None
                    else None
                ),
                full_batch_tail_lens_cpu=full_tail_lens_cpu,
            )
        else:
    ...
    def enter_late_layer_tail(self, forward_batch: ForwardBatch) -> tuple:
        ...
        if tail.cp_metadata is None and tail.full_batch_tail_lens_cpu is not None:
            # DSV41 opt1 mixed batch: cut per full-batch request (a dropped
            # request keeps an EMPTY entry, not a missing one) so the list
            # stays bs-aligned for the late layers' consume-by-request-index
            # lookups; dropped requests contribute no rows anyway.
            tail_lens_cpu = tail.full_batch_tail_lens_cpu
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch) -> None:
        ...
        # DSV41 opt1: the scheduler marks which extend chunks finish their
        # request (ForwardBatch.extend_last_chunk_mask). With the env switch
        # on, non-final rows are dropped from the tail (their late-layer output
        # is dead), and a batch with no final request skips the tail entirely.
        # CP-v2 keeps the base layout (its tail rows are reshuffled per rank).
        final_req_mask = (
            forward_batch.extend_last_chunk_mask
            if self.enable_decoder_swa_bounded_replay
            and self.skip_nonfinal_decoder_tail
            and forward_batch.forward_mode.is_extend_without_speculative()
            and not is_cp_v2_active(forward_batch)
            else None
        )
        self.late_layer_tail_skip = False
        build_tail = (
            self.enable_decoder_swa_bounded_replay
            and forward_batch.forward_mode.is_extend_without_speculative()
        )
        if build_tail and final_req_mask is not None and not any(final_req_mask):
            # No request finishes its prefill in this chunk: nothing reads
            # the late layers' output (logits of middle chunks are
            # discarded and their window KV is overwritten by later
            # chunks), so skip the tail forward altogether.
            self.tail_forward_metadata = None
            self.late_layer_tail_skip = True
        elif build_tail:
            self.tail_forward_metadata = self._build_late_layer_tail_metadata(
                forward_batch, final_req_mask=final_req_mask
            )
        else:
            self.tail_forward_metadata = None
    ...
    def forward_low_ratio_sources(
        ...
        *,
        ...
        if (
            ...
            and getattr(
                self.forward_metadata.late_layer_tail,
                "full_batch_tail_lens_cpu",
                None,
            )
            is None
        ):
    ...
    def _low_ratio_index_topk_extend(
        ...
        if tail is not None:
            ...
            if tail.full_batch_tail_lens_cpu is not None:
                # DSV41 opt1 mixed batch: this tail covers only the final-chunk
                # requests, but _low_ratio_index_topk_dense derives starts,
                # lc_per_req and req_pool_indices from the FULL batch. Re-key
                # the lens onto the full batch (dropped requests repeat 0
                # times); otherwise the ks repeat_interleave gets
                # len(repeats) == len(keep) != len(starts) == bs and the kernel
                # reads repeats past its end (device assert
                # "output_size != sum(repeats)") or mis-offsets kept rows.
                q_lens = tail.full_batch_tail_lens
                q_lens_cpu = tail.full_batch_tail_lens_cpu
        else:
    ...
    def _low_ratio_index_topk_dense(
        ...
        # Port of upstream PR #39187 onto this tree's candidate_masks convention:
        # row-chunk the dense logits under a fixed byte budget and compute the
        # candidate block masks only for the late-layer tail rows, so the dense
        # prefill indexer transient is bounded (2 GiB logits budget) instead of
        # num_tokens x max_lc.
        publish = [] if indexer.is_candidate_source else None
        consume = self.candidate_masks if (
            indexer.uses_candidates and publish is None
        ) else None
        ...
        if (
            ...
            tail_rows = tail_metadata.late_layer_tail.extend_seq_lens_cpu
            # Tail rows map 1:1 onto this forward batch only when every
            # request in the batch carries decoder rows (all-final chunk
            # batch). Mixed batches build a subset tail whose indices do not
            # address this loop's requests, and non-final skip batches build
            # no tail at all -- both fall back to full-rows publish.
            publish_rows_per_req = (
                tail_rows if len(tail_rows) == len(q_lens_cpu) else None
            )
        ...
        for b, (lc, t_len) in enumerate(zip(lc_per_req, q_lens_cpu)):
            ...
            for c0 in range(0, t_len, rows_per_chunk):
                ...
                if consume is not None:
                    selected[rows] = _mask_topk_scores(logits, selected[rows], ks[rows])
                del logits, scores
            if publish is not None:
    ...
    def _flash_mla_sm120_once(
        ...
        """One FlashInfer SM120/121 sparse-MLA call; the fp4 repack bridge and
        its query slicing (M1) call this per slice with bridged extras."""
        if extra_k_cache is not None:
            assert extra_k_cache.shape[-1] != KVLayout.V41_FP4.bytes_per_token, (
                f"a raw {extra_k_cache.shape[-1]} B/token V41_FP4 extra cache "
                "reached the SM120/121 sparse-MLA kernel; it reads the 584-byte V4 "
                "form only, so the pool must go through repack_fp4_extra_to_v4"
            )
        return kernel(
            q=q,
            k_cache=k_cache,
    ...
    def _forward_attention(
        ...
        **_,
        ...
        if isinstance(core_attn_metadata, DSV4AttnMetadata):
            ...
            # The reader picks the format from the last dim of this view
            # (584 = V4, 528 = V4.1 fp8). It has no fp4 form: a 288 B/token
            # V41_FP4 extra cache must be bridged into a V4 scratch before the
            # call below and is never handed to the kernel raw.
            ...
            if get_platform().is_sm120:
                ...
                # FORK (M1 FP4 main-KV repack bridge): the FlashInfer SM120/121
                # sparse-MLA kernel only reads the 584-byte V4 layout, so a
                # V41_FP4 extra cache is gathered into a persistent V4 scratch
                # (repack_fp4_to_v4.py; bit-exact with the pre-fork fp8 pool
                # rows) and the indices are rebased onto it -- the kernel call
                # below is unchanged. Decode/verify batches fit one slice;
                # larger extends are query-sliced under the scratch row budget
                # (the softmax is per query token, so slicing is exact).
                fp4_bridge = extra_k_cache is not None and (
                ...
                if fp4_bridge:
                    from sglang.kernels.ops.attention.dsv4.repack_fp4_to_v4 import (
                        bridge_note_call,
                        bridge_note_slices,
                        bridge_slice_bounds,
                        bridge_stats_period,
                        bridge_time_enabled,
                        repack_fp4_extra_to_v4,
                    )

                    slice_bounds = bridge_slice_bounds(
                        q.shape[0], extra_indices.shape[-1]
                    )
                    bridge_note_slices(len(slice_bounds) - 1)
                    # Telemetry only (DSV41_FP4_BRIDGE_STATS, off by default).
                    # real_rows needs a host sync on topk_lengths, so it is taken
                    # for eager extends only -- never inside graph capture.
                    _nq = q.shape[0]
                    _topk = int(extra_indices.shape[-1])
                    _real, _t0, _tmid, _t1 = -1, None, None, None
                    if (
                        extra_topk_lengths is not None
                        and _nq > SM120_DECODE_MAX_TOKENS
                        and not torch.cuda.is_current_stream_capturing()
                        and bridge_time_enabled()
                    ):
                        _real = int(extra_topk_lengths.clamp(max=_topk).sum().item())
                        _t0 = torch.cuda.Event(enable_timing=True)
                        _tmid = torch.cuda.Event(enable_timing=True)
                        _t1 = torch.cuda.Event(enable_timing=True)
                        _t0.record()
                    if len(slice_bounds) == 2:
                        extra_k_cache, extra_indices = repack_fp4_extra_to_v4(
                            extra_k_cache, extra_indices, extra_page_size
                        )
                        if _tmid is not None:
                            _tmid.record()
                        o = self._flash_mla_sm120_once(
                            q,
                            swa_k_cache,
                            swa_page_indices,
                            swa_topk_lengths,
                        ...
                        parts = []
                        for lo, hi in zip(slice_bounds[:-1], slice_bounds[1:]):
                            scratch, bridged = repack_fp4_extra_to_v4(
                                extra_k_cache, extra_indices[lo:hi], extra_page_size
                            )
                            parts.append(
                                self._flash_mla_sm120_once(
                                    q[lo:hi],
                                    swa_k_cache,
                                    swa_page_indices[lo:hi],
                                    swa_topk_lengths[lo:hi],
                                    attn_sink,
                                    scratch,
                                    bridged,
                                    (
                                        extra_topk_lengths[lo:hi]
                    ...
                    if _t0 is None:
                        if bridge_stats_period():
                            bridge_note_call(_nq, _topk, -1, 0.0, 0.0)
                    else:
                        _t1.record()
                        torch.cuda.synchronize()
                        bridge_note_call(
                            _nq, _topk, _real,
                            _t0.elapsed_time(_tmid) * 1000.0,
                            _tmid.elapsed_time(_t1) * 1000.0,
                        )
                else:
                    o = self._flash_mla_sm120_once(
                        q,
                ...
                if extra_k_cache is not None:
                    assert (
                        token_to_kv_pool.get_extra_key_layout(layer_id)
                        is not KVLayout.V41_FP4
                    ), (
                        "the non-SM120 sparse-MLA path reads the 584-byte V4 extra "
                        "layout and cannot consume a V41_FP4 pool; the fp4 bridge "
                        "is SM120/121-only"
                    )
                if _is_xpu:
