...
`IMAGE` 的值在三个地方出现，**任一处落后都会静默生效**：

| # | 位置 | 作用 |
|---|---|---|
| 1 | `.env.tp4`（生产） | 实际被 `start-tp4.sh` 读取的值 |
| 2 | `.env.tp4.example`（本仓） | 新部署从它复制；落后 ⇒ 照模板起栈拉到旧镜像 |
| 3 | `guards.conf` 的 `sglang-image` envpin | 守卫比对面；三处不一致时 **preflight 拦下** |

**这处曾经真的落后过**：模板与 `start.sh` 的默认值在 2026-09-23 之前仍停在
`v7`（0.1.7 世代，125 层）与 `0.2.7`，而生产已在 `0.2.8`——"照模板部署"与
"生产实况"之间**没有任何告警面**。本轮的修正即为此。

⚠️ `start.sh` 的 `IMAGE` 默认值（`dsv41-3x-spark:local`）服务的是 **3-Spark profile**
（`.env`），**不是** TP4；TP4 一律由 `.env.tp4` 显式提供 `IMAGE`，故默认值不参与
TP4 起栈。**不要**为了让 TP4 好看而改它——那会改掉 3-Spark 的行为。
